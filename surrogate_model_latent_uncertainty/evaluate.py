"""
surrogate_model_latent_uncertainty/evaluate.py
-------------------------------------------------
Evaluation and visualisation for the trained LPBF Latent-Space Gaussian
Ensemble surrogate.

Plot style matches surrogate_model_latent/evaluate.py exactly:
...
-------
  ...
  3. Per-layer mean epistemic σ  (ensemble disagreement) and aleatoric σ
     (average member noise) from GaussianTransitionMLP
...
------------------------------------------------------------
  ...
  traj_<i>/sigma_per_layer.png    — epistemic & aleatoric curves for this trajectory
...
from surrogate_model_latent_uncertainty.train import (
    load_latent_surrogate, compute_single_step_losses, weighted_mse,
)


# =============================================================================
# Mesh loading  (identical to surrogate_model_latent/evaluate.py)
# =============================================================================
...
def evaluate_single_step(
    ...
    for traj_s, traj_a in test_loader:
        ...
        for t in range(min(T, traj_len)):
            ...
            mu_mean, _, _, _ = model.predict_ensemble(z_t, a_in, layer_idx)
...
def evaluate_rollout(
    ...
    per_layer_epistemic  : (traj_len,)   mean epistemic σ per layer
    per_layer_aleatoric  : (traj_len,)   mean aleatoric σ per layer
    ...
    all_epistemic        : list of (T,)   np.ndarray  — epistemic σ per layer [latent space]
    all_aleatoric        : list of (T,)   np.ndarray  — aleatoric σ per layer [latent space]
    """
    model.eval()
    ...
    for traj_s, traj_a in test_loader:
        ...
        pred_states, epistemic_stds, aleatoric_stds = model.rollout(
            traj_s[:, 0, :], traj_a[:, :T, :]
        )
        # pred_states : (B, T, D)   epistemic_stds, aleatoric_stds : (B, T)
        ...
        epi_np     = epistemic_stds.cpu().numpy()                               # (B, T)
        ale_np     = aleatoric_stds.cpu().numpy()                               # (B, T)

        for b in range(B):
            all_preds.append(pred_np[b])
            all_gts.append(gt_np[b])
            all_actions.append(actions_np[b])
            all_epistemic.append(epi_np[b])
            all_aleatoric.append(ale_np[b])
    ...
    per_layer_epistemic = np.array([np.mean([s[t] for s in all_epistemic])  for t in range(traj_len)])
    per_layer_aleatoric = np.array([np.mean([s[t] for s in all_aleatoric])  for t in range(traj_len)])

    return (per_layer_rmse, per_layer_mae, per_layer_epistemic, per_layer_aleatoric,
            all_preds, all_gts, all_actions, all_epistemic, all_aleatoric)
...
def evaluate_loss_components(
    ...
    n   = 0
    for s, a, s2, layer_idx, bmask in flat_loader:
        s         = s.to(device)
        a         = a.to(device)
        s2        = s2.to(device)
...
# =============================================================================
# Plotting helpers  (matched to surrogate_model_latent/evaluate.py style)
# =============================================================================
...
def plot_sigma_sequence(
    epistemic: np.ndarray,   # (T,) mean epistemic σ per layer
    aleatoric: np.ndarray,   # (T,) mean aleatoric σ per layer
    traj_idx:  int,
    out_path:  str,
) -> None:
    """Per-layer epistemic vs aleatoric uncertainty for one trajectory."""
    T      = len(epistemic)
    layers = np.arange(1, T + 1)
    fig, ax = plt.subplots(figsize=(9, 3))
    ax.plot(layers, epistemic, marker="o", color="tab:orange", linewidth=1.5,
            label="Epistemic (ensemble disagreement)")
    ax.plot(layers, aleatoric, marker="s", color="tab:purple", linewidth=1.5,
            label="Aleatoric (avg. member noise)")
    ax.set_xlabel("Layer")
    ax.set_ylabel("Mean latent σ")
    ax.set_title(f"Trajectory {traj_idx} — Per-layer Uncertainty")
    ax.set_xticks(layers)
    ax.legend(fontsize=8)
    ax.grid(True, alpha=0.3)
    fig.tight_layout()
...
def plot_example_trajectory(
    ...
    # ── sigma curve ───────────────────────────────────────────────────────────
    plot_sigma_sequence(epistemic, aleatoric, traj_idx,
...
def plot_per_layer_uncertainty(
    epistemic_results:  dict,   # {split_name: per_layer_epistemic}
    aleatoric_results:  dict,   # {split_name: per_layer_aleatoric}
    out_path: str,
) -> None:
    """Epistemic vs aleatoric uncertainty per layer for each split, 3 panels
    (epistemic, aleatoric, total)."""
    traj_len = len(next(iter(epistemic_results.values())))
    layers   = np.arange(1, traj_len + 1)

    fig, axes = plt.subplots(1, 3, figsize=(16, 4.5))

    for split, sigma in epistemic_results.items():
        color, marker, ls = _SPLIT_STYLES.get(split, ("grey", "o", "-"))
        axes[0].plot(layers, sigma, marker=marker, color=color, linestyle=ls,
                     linewidth=1.5, label=split)
    axes[0].set_title("Epistemic σ (ensemble disagreement)", fontsize=9)

    for split, sigma in aleatoric_results.items():
        color, marker, ls = _SPLIT_STYLES.get(split, ("grey", "o", "-"))
        axes[1].plot(layers, sigma, marker=marker, color=color, linestyle=ls,
                     linewidth=1.5, label=split)
    axes[1].set_title("Aleatoric σ (avg. member noise)", fontsize=9)

    for split in epistemic_results:
        total = np.sqrt(epistemic_results[split] ** 2 + aleatoric_results[split] ** 2)
        color, marker, ls = _SPLIT_STYLES.get(split, ("grey", "o", "-"))
        axes[2].plot(layers, total, marker=marker, color=color, linestyle=ls,
                     linewidth=1.5, label=split)
    axes[2].set_title("Total σ (moment-matched mixture)", fontsize=9)
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Evaluate the LPBF Latent-Space Gaussian Ensemble surrogate "
                    "on held-out test trajectories."
    )
    ...
    p.add_argument("--n_ensemble",    type=int,   default=5,
                   help="Must match the checkpoint's ensemble size "
                        "(only used to reconstruct bootstrap masks for loss reporting).")
...
def main() -> None:
    ...
    def _traj_loader_only(trajs):
        """LatentTrajectoryDataset yields (s, a, bmask); most metric functions here
        only need (s, a), so wrap the loader to drop the bootstrap mask."""
        base = DataLoader(LatentTrajectoryDataset(trajs, **ds_kwargs, n_ensemble=model.n_ensemble,
                                                    bootstrap_seed=args.seed), **loader_kw)
        return _DropLastColumn(base)

    split_loaders = {
        "train": (_traj_loader_only(train_trajs),
                  DataLoader(LatentSurrogateDataset(train_trajs, **flat_kwargs), **loader_kw)),
        "val":   (_traj_loader_only(val_trajs),
                  DataLoader(LatentSurrogateDataset(val_trajs,   **flat_kwargs), **loader_kw)),
        "test":  (_traj_loader_only(test_trajs),
                  DataLoader(LatentSurrogateDataset(test_trajs,  **flat_kwargs), **loader_kw)),
    }

    # ── evaluate each split ───────────────────────────────────────────────────
    rmse_results  = {}   # {split: (ro_rmse, ss_rmse)}
    mae_results   = {}   # {split: (ro_mae, ss_mae)}
    epistemic_results = {}   # {split: ro_epistemic}
    aleatoric_results = {}   # {split: ro_aleatoric}

    summary = {}   # {split: {metric: value}}

    test_preds, test_gts, test_actions = None, None, None
    test_epistemic, test_aleatoric = None, None
    ...
    for split_name, (traj_loader, flat_loader) in split_loaders.items():
        ...
        (ro_rmse, ro_mae, ro_epi, ro_ale,
         all_preds, all_gts, all_actions, all_epi, all_ale) = evaluate_rollout(
            model, traj_loader,
            state_mean_cpu, state_std_cpu,
            action_mean, action_std,
            device, traj_len=traj_len,
        )
        print(f"\n  {'Layer':>6}  {'RO RMSE [K]':>12}  {'RO MAE [K]':>12}  "
              f"{'Epist σ':>10}  {'Aleat σ':>10}")
        for i in range(traj_len):
            print(f"  {i+1:>6}  {ro_rmse[i]:>12.3f}  {ro_mae[i]:>12.3f}  "
                  f"{ro_epi[i]:>10.4f}  {ro_ale[i]:>10.4f}")
        print(f"  {'Mean':>6}  {ro_rmse.mean():>12.3f}  {ro_mae.mean():>12.3f}  "
              f"{ro_epi.mean():>10.4f}  {ro_ale.mean():>10.4f}")

        rmse_results[split_name]      = (ro_rmse, ss_rmse)
        mae_results[split_name]       = (ro_mae,  ss_mae)
        epistemic_results[split_name] = ro_epi
        aleatoric_results[split_name] = ro_ale
        ...
        summary[split_name] = {
            ...
            "epist_std":       ro_epi.mean(),
            "aleat_std":       ro_ale.mean(),
        }

        if split_name == "test":
            test_preds, test_gts     = all_preds, all_gts
            test_actions              = all_actions
            test_epistemic            = all_epi
            test_aleatoric            = all_ale

    # ── consolidated summary table ────────────────────────────────────────────
    metrics = ["recon_rmse [K]", "ss_mae [K]", "ss_rmse [K]", "ro_mae [K]", "ro_rmse [K]",
               "epist_std", "aleat_std"]
    col_w   = 14
    ...
    plot_mae_per_layer(mae_results,
                       ...
                       pct_refs=pct_refs)
    plot_per_layer_uncertainty(epistemic_results, aleatoric_results,
    ...
    for i in range(n_ex):
        plot_example_trajectory(
            ...
            epistemic=test_epistemic[i],
            aleatoric=test_aleatoric[i],
            traj_idx=i,
            out_dir=out_dir,
            triang=triang,
            vmin=args.vmin,
...
class _DropLastColumn:
    """Wraps a DataLoader that yields (s, a, bmask) tuples and drops bmask,
    so metric functions written for (s, a) can be reused unchanged."""
    def __init__(self, loader: DataLoader):
        self._loader = loader

    def __iter__(self):
        for s, a, _bmask in self._loader:
            yield s, a
