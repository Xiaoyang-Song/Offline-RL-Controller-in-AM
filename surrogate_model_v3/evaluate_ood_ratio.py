"""
surrogate_model_v3/evaluate_ood_ratio.py
------------------------------------------------
...
IMPORTANT: `--checkpoint_full` must be a MATCHED control — same
architecture/K/bootstrap_frac/perturb_frac as the patchy run, same
--data_path, just WITHOUT any --lp_filter_* flag. If perturb_frac/
bootstrap_frac differ between the two checkpoints, the ratio bakes in that
confound instead of isolating the coverage effect (see
jobs/train_full_matched.sh).
...
from surrogate_model_v3.evaluate_ood import collect_ood_samples, bin_by_action
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--checkpoint_patchy", type=str, required=True,
                   help="Checkpoint trained on a restricted/gapped laser-power range "
                        "(train.py's --lp_filter_min/--lp_filter_max or --lp_filter_ranges).")
    p.add_argument("--checkpoint_full", type=str, required=True,
                   help="Checkpoint trained on the SAME --data_path with NO "
                        "--lp_filter_min/--lp_filter_max/--lp_filter_ranges, and the SAME "
                        "perturb_frac/bootstrap_frac as the patchy run (matched control).")
    p.add_argument("--data_path", type=str, required=True,
                   help="The wide dataset both checkpoints are evaluated on.")
    p.add_argument("--id_action_min", type=float, default=150.0,
                   help="Lower bound of the patchy checkpoint's training range [W]. "
                        "Ignored if --id_ranges is given.")
    p.add_argument("--id_action_max", type=float, default=300.0,
                   help="Upper bound of the patchy checkpoint's training range [W]. "
                        "Ignored if --id_ranges is given.")
