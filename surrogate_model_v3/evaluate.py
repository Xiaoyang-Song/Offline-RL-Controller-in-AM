"""
surrogate_model_v3/evaluate.py
------------------------------------
Evaluation and visualisation for the trained two-stage (heating/cooling)
LPBF Gaussian ensemble surrogate (no latent space).

Plot style matches the sibling packages: jet colormap, caxis [300, 5000],
Gouraud shading, triplot mesh overlay, hot colormap for error field.
...
-------
  ...
  2. Auto-regressive rollout MAE / RMSE per layer, heating stage and cooling stage
  3. Per-layer mean epistemic / aleatoric / total σ, heating / cooling / combined
  4. Per-component test-set loss (recon_heat, nll_heat, recon_cool, nll_cool)
...
------------------------------------------------------------
  ...
  per_layer_uncertainty.png      — 3 rows (heating/cooling/combined) × 3 cols (epistemic/aleatoric/total)
  traj_<i>/actions.txt
...
-----
    python -m surrogate_model_v3.evaluate \\
        --checkpoint surrogate_model_v3/runs/<ts>/surrogate_best.pt \\
...
from surrogate_model_v3.dataset import (
    load_trajectories, split_trajectories, TwoStageTrajectoryDataset, TwoStageSurrogateDataset,
)
from surrogate_model_v3.model import load_surrogate
from surrogate_model_v3.train import compute_losses
...
def evaluate_single_step(
    ...
    input (cooling stage conditions on h_gt, not the heating stage's
    ...
    for traj_s, traj_h, traj_a, traj_c in traj_loader:
        ...
        for t in range(T):
            ...
            mu_heat, _, _, _ = model.predict_heating_ensemble(s_in, a_in, layer_idx)
            heat_pred = s_in + mu_heat
            ...
            mu_cool, _, _, _ = model.predict_cooling_ensemble(h_gt, c_in, layer_idx)
            next_pred = h_gt + mu_cool
...
@torch.no_grad()
def evaluate_loss_components(model, flat_loader, device) -> dict:
...
def plot_sigma_sequence(heat_epi, heat_ale, cool_epi, cool_ale, traj_idx, out_path):
    ...
    axes[0].set_ylabel("Mean σ [K]"); axes[0].legend(fontsize=8); axes[0].grid(True, alpha=0.3)
    ...
    axes[1].set_ylabel("Mean σ [K]"); axes[1].legend(fontsize=8); axes[1].grid(True, alpha=0.3)
...
def plot_per_layer_uncertainty(results, out_path):
    ...
    for i in range(3):
        for j in range(3):
            ...
            ax.set_xlabel("Layer index"); ax.set_ylabel("Mean σ [K]")
            ax.legend(fontsize=8); ax.grid(True, alpha=0.3); ax.set_xticks(layers)

    fig.suptitle("Uncertainty per Layer — Train / Val / Test", fontsize=11)
    fig.tight_layout(); fig.savefig(out_path, dpi=150); plt.close(fig)
    print(f"[evaluate] Saved → {out_path}")
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Evaluate the two-stage LPBF Gaussian ensemble surrogate (no latent space)."
    )
...
def main() -> None:
    ...
    def _traj_loader_only(trajs):
        base = DataLoader(
            TwoStageTrajectoryDataset(trajs, **ds_kwargs, n_ensemble=model.n_ensemble,
    ...
    split_loaders = {
        "train": (_traj_loader_only(train_trajs),
                  DataLoader(TwoStageSurrogateDataset(train_trajs, **flat_kwargs), **loader_kw)),
        "val":   (_traj_loader_only(val_trajs),
                  DataLoader(TwoStageSurrogateDataset(val_trajs,   **flat_kwargs), **loader_kw)),
        "test":  (_traj_loader_only(test_trajs),
                  DataLoader(TwoStageSurrogateDataset(test_trajs,  **flat_kwargs), **loader_kw)),
    }
    ...
    for split_name, (traj_loader, flat_loader) in split_loaders.items():
        ...
        comps = evaluate_loss_components(model, flat_loader, device)
