"""
baseline_surrogate/vanilla_ensemble/train.py
--------------------------------------------------
Train the vanilla deep ensemble baseline (baseline_surrogate/vanilla_ensemble/model.py).
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Train the vanilla-deep-ensemble surrogate baseline.")
    ...
    p.add_argument("--out_dir", type=str, default="baseline_surrogate/vanilla_ensemble/runs/default")
...
def main() -> None:
    ...
    print(f"[vanilla_ensemble.train] Output dir : {args.out_dir}")
    print(f"[vanilla_ensemble.train] Device     : {device}")
    ...
    if args.lp_filter_min is not None or args.lp_filter_max is not None:
        ...
        print(f"[vanilla_ensemble.train] LP filter active: [{args.lp_filter_min}, {args.lp_filter_max}] W")
    ...
    print(f"[vanilla_ensemble.train] VanillaDeepEnsembleSurrogate params={model.count_parameters():,}")

    optimizer = torch.optim.AdamW(model.parameters(), lr=args.lr, weight_decay=args.weight_decay)
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=args.epochs, eta_min=args.lr / 100)

    ckpt_best = os.path.join(args.out_dir, "vanilla_ensemble_best.pt")
    ...
    run_training(model, train_loader, val_loader, loss_fn, optimizer, device,
                 ...
                 save_best_fn=save_best, log_prefix="[vanilla_ensemble.train] ")

    print(f"[vanilla_ensemble.train] Done. Best checkpoint: {ckpt_best}")
