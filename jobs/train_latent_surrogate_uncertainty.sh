#!/bin/bash
# =============================================================================
# train_latent_surrogate_uncertainty.sh
# SLURM job script — train the LPBF Latent-Space Gaussian Ensemble Dynamics
# surrogate model (Gaussian transition heads + bootstrap-resampled ensemble)
#
# Submit:
#   sbatch jobs/train_latent_surrogate_uncertainty.sh
#
# Override any arg at submission time, e.g.:
#   sbatch jobs/train_latent_surrogate_uncertainty.sh --latent_dim 128 --epochs 300
# =============================================================================

#SBATCH --job-name=latent_surrogate_uncertainty
...
#SBATCH --time=2:00:00
#SBATCH --output=/nfs/turbo/coe-sunwbgt/xysong/Offline-RL-Controller-in-AM/jobs/latent_surrogate_uncertainty_%j.log
...
# Bootstrap ensemble
BOOTSTRAP_SEED=-1   # -1 → reuse SEED below

# Loss weights
RECON_ST_WEIGHT=1.0   # autoencoder reconstruction of s_t
RECON_ST1_WEIGHT=1.0  # per-member predicted s_{t+1} reconstruction (bootstrap-weighted mean over K)
NLL_WEIGHT=0.1         # per-member latent Gaussian NLL (calibrates aleatoric uncertainty)
